...
namespace ptl {
	class string final { //TODO: [C++20] constexpr
		class rep_t final {
			static
			constexpr
			std::size_t sso_size{((sizeof(void * ) * 3) / sizeof(char)) - 2};
		...
		public:
			...
			void swap(rep_t & other) noexcept { //TODO: [C++??] precondition(this != std::addressof(other));
				if(dealloc && other.dealloc) {
					...
					std::swap_ranges(sso.buf, sso.buf + sso_size + 1, other.sso.buf); //TODO: technically only max(sso.siz, other.sso.siz) must be exchanged
				} else {
